...
- Any ambiguity flag (hands, occlusion, etc.): START CAPTURING EVENT ✅
- Flags end → SUBMIT entire 8-frame clip showing progression
- VLM sees: "what happened in this 2-second scene?"

TRIGGERS:
1. prolonged_closure (EAR < 0.26 for 2+ seconds)
2. yawning + ambiguity_flags (MAR > 0.60 with suspicious conditions)
3. ANY ambiguity flag alone (no_face, extreme_low_ear, busy_edges, very_dark, overexposed)
...
def detect_ambiguity_flags(frame_bgr, face_detected, ear, mar, brightness, face_conf):
    ...
    if face_detected:
        ...
        if edge_density > CONFIG["edge_density_threshold"]:
            flags.append("busy_edges")  # ← HANDS, SUNGLASSES, OCCLUSION DETECTED HERE
...
class HybridDrowsinessDetector:
    def __init__(self, camera_id=0, driver_id="driver1", model_path="face_landmarker.task", output_dir="./vlm_triggers"):
        ...
        print(f"✅ TRIGGERS: Drowsiness + Yawning + ANY Ambiguity (hands, occlusion, etc.)")
    ...
    def _process_frame(self, frame):
        ...
        # TRIGGER ON:
        # 1. Prolonged eye closure (drowsiness)
        # 2. Yawning + ambiguity
        # 3. ANY ambiguity flag (hands on face, occlusion, busy_edges, no_face, etc.)
        has_flags = bool(prolonged_closure or (yawning and ambiguity_flags) or bool(ambiguity_flags))
        ...
        # Mode info
        cv2.putText(disp, f"⚡ EVENT CAPTURE MODE: Triggers on drowsiness, yawning, OR ANY occlusion", (10, 150),
    ...
    def run_camera(self):
        ...
        print("🚩 TRIGGERS (any of these will start event capture):")
        print("   1. Prolonged eye closure (2+ seconds)")
        print("   2. Yawning + ambiguity detected")
        print("   3. Hands on face / busy edges / occlusion")
        print("   4. No face detected / extreme low EAR")
        print("   5. Very dark / overexposed lighting\n")
